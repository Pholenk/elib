	...
	<li class="cpanel-menu-text active">Anggota
    	<div class="menu-kiri-content">
            ...
            <a href=cpanel-admin-anggota-verifikasi.php>Verifikasi Anggota</a>
       		...
       		<a class="active" href=cpanel-admin-anggota-hapus.php>Hapus Data Anggota</a>
...
<div class="content">
	<div class="formulir">
		<h2 style="text-align: center; margin:0; padding:15px 0; font-family: 'Arial Black', Gadget, sans-serif;">Hapus Anggota</h2>
    		<form class="registrasi" name="registrasi" method="post" action="">
				<table>
					<tr>
						<td width="30%"><span class="formulir-text">ID Anggota</span>
						</td>
						<td class="regis">
							<span class="formulir-text">
        	    		    	<input type="text" name="id_anggota" class="input-textfield" />
			                </span>
			            </td>
					</tr>
				</table><br><br>
                ...
                <?php
                  if($_POST)
                  {
                    if(empty($_POST["id_anggota"]))
                    {
                      ...
                      document.location="cpanel-admin-anggota-hapus.php";
                      </script>
                      <?php
                    }
                    else
                    {
                      if($in->cek_anggota($_POST["id_anggota"])==TRUE)
                      {
                        if($in->hapusdetail_anggota($_POST["id_anggota"])==TRUE)
                        {
                          ?>
                          <script language="javascript">
